// google-account-disconnect: Revoke Google OAuth, delete Vault secret, mark account revoked.
// Optionally purges collected messages if purgeMessages=true.
// Uses lifecycle_disconnect RPC for atomic disconnect with reconnect race detection.
...
async function revokeGoogleToken(token: string): Promise<{ ok: boolean; terminal: boolean }> {
  try {
    ...
    // 400/401/403 = token already dead — treat as success (no live grant to worry about)
    if (res.status === 400 || res.status === 401 || res.status === 403) {
      return { ok: true, terminal: true }
    }
    // 5xx = Google-side error — block cleanup, user can retry
    return { ok: false, terminal: false }
  } catch {
    // Network error — block cleanup
    return { ok: false, terminal: false }
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  let purgeMessages: boolean
  try {
    const body = await req.json() as { accountId?: unknown; purgeMessages?: unknown }
    ...
    purgeMessages = body.purgeMessages === true
  } catch {
  ...
  if (vaultErr) {
    // Vault RPC error (not null result) — do not proceed
  ...
  // refreshToken === null means secret is genuinely missing — proceed with empty token (Google will 400, treat as terminal)
  const tokenToRevoke: string = refreshToken ?? ''

  // Revoke Google token
  const revoke = await revokeGoogleToken(tokenToRevoke)
  if (!revoke.ok) {
    // 5xx or network error — nothing changed yet; Vault token retained; user can retry
  ...
  const { data: disconnected, error: rpcErr } = await supabaseAdmin
    .rpc('lifecycle_disconnect', {
      ...
      p_user_id: user.id,
      p_purge: purgeMessages,
      p_expected_version: account.lifecycle_version,
    })

  if (rpcErr) {
    console.error('lifecycle_disconnect RPC error:', JSON.stringify(rpcErr))
    return Response.json({ error: 'Disconnect failed' }, { status: 500, headers: corsHeaders })
  }
